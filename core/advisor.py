...
class Advisor:
    ...
    async def advise(self, model, crisis_text, policy_options, state_dict, thread, policy_base_effects_list):
        prompt = (
        ...
        prompt += (
            ...
            f"You should suggest a specific allocation (e.g., 50% to A, 30% to B, 20% to C), or argue for prioritizing certaain options.\n"
...
class Council:
    ...
    async def consult(self, model, crisis_text, policy_options, state_dict, thread, policy_base_effects_list):
        responses = []
        for advisor in self.advisors:
            response = await advisor.advise(model, crisis_text, policy_options, state_dict, thread, policy_base_effects_list)
            responses.append((advisor.name, response))
        return responses
    
    def update_influence(self):
        for advisor in self.advisors:
            advisor.influence += random.randint(1, 5)
